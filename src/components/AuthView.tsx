...
export default function AuthView({ onLogin }: AuthViewProps) {
  ...
  return (
    <div className="flex h-screen w-screen items-center justify-center bg-slate-900 relative overflow-hidden z-50">
      ...
      <div className="w-full max-w-md p-8 glass-panel border border-white/10 rounded-2xl shadow-2xl relative z-10 backdrop-blur-xl bg-white/5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!isLogin && (
            <div>
              ...
              <input
                ...
                className="w-full bg-black/20 border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium py-2.5 rounded-lg transition-colors shadow-lg shadow-blue-500/25 mt-6 disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isLoading && (
