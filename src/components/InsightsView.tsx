    ...
    <div className="flex-1 overflow-y-auto bg-transparent p-6 scrollbar-thin" id="insights-workspace">
      ...
      <div className="max-w-5xl mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-slate-900/5 backdrop-blur-md rounded-2xl border border-slate-900/10 p-5 shadow-xs flex items-center gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-900/5 backdrop-blur-md rounded-2xl border border-slate-900/10 p-6 shadow-xs flex flex-col justify-between">
            <div>
            ...
            <div className="space-y-4">
              {/* Giant composite bar */}
              <div className="h-6 w-full rounded-full overflow-hidden flex bg-slate-900/5 border border-slate-900/10">
                {donePoints > 0 && (
                  <div
              ...
              <div className="grid grid-cols-3 gap-2.5 text-xs">
                ...
                <div className="flex items-center gap-1.5 p-2 bg-slate-900/5 rounded-xl border border-slate-900/10 text-slate-500">
          ...
          <div className="bg-slate-900/5 backdrop-blur-md rounded-2xl border border-slate-900/10 p-6 shadow-xs flex flex-col md:flex-row md:items-center gap-6">
        ...
        <div className="bg-slate-900/5 backdrop-blur-md rounded-2xl border border-slate-900/10 p-6 shadow-xs">
              ...
              velocityData.map((data, index) => {
                ...
                return (
                  <div key={index} className="space-y-1.5 text-xs">
                    ...
                    <div className="h-4.5 w-full bg-slate-900/5 rounded-full overflow-hidden relative border border-slate-900/10">
                      {/* total points range bar background */}
                      <div
                        style={{ width: `${Math.min(100, (data.totalPoints / 30) * 100)}%` }}
                        className="h-full bg-slate-900/5"
                      >
                        {/* done segment */}
                        <div
                      ...
                      {/* fallback text */}
                      {data.totalPoints === 0 && (
                        <span className="absolute inset-0 flex items-center justify-center text-[10px] text-slate-500 font-bold">0 story points planned</span>
                      )}
                    </div>
                  </div>
                );
        ...
        <div className="bg-slate-900/5 border border-slate-900/10 backdrop-blur-md rounded-2xl overflow-hidden shadow-xs">
          <div className="px-5 py-4.5 border-b border-slate-900/10 bg-slate-900/5 text-slate-800 flex items-center justify-between">
            <div>
            ...
            <span className="text-xs bg-blue-600 text-slate-900 font-bold px-3.5 py-1.5 rounded-full shadow-xs">
              {totalHoursLogged} Hrs Logged
            </span>
          </div>

          {allWorkLogs.length === 0 ? (
            <div className="p-8 text-center text-xs text-slate-500 italic bg-slate-900/5 font-semibold">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-900/5 border-b border-slate-900/10 text-slate-500 font-bold uppercase tracking-wider text-[10px]">
                ...
                <tbody className="divide-y divide-white/20">
                  {allWorkLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-900/5 transition-colors">
