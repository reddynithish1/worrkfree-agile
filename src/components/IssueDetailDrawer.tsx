...
export default function IssueDetailDrawer({
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-40 w-full max-w-4xl bg-white/20 backdrop-blur-2xl shadow-2xl border-l border-white/30 flex flex-col animate-in slide-in-from-right duration-250">
      
      {/* Drawer Title & Actions Bar */}
      <div className="px-6 py-4.5 border-b border-white/20 bg-white/20 text-slate-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="p-2 hover:bg-white/20 text-rose-600 hover:text-rose-700 rounded-full transition-colors cursor-pointer"
            title="Delete Issue"
          >
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-white/20 text-slate-500 hover:text-slate-800 rounded-full transition-colors cursor-pointer"
          >
      ...
      <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 scrollbar-thin">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div>
            <input
              ...
              className="w-full text-xl font-bold text-slate-800 border-0 border-b border-transparent hover:border-white/40 focus:border-blue-500 focus:ring-0 px-2 py-1.5 transition-all bg-transparent rounded-xl focus:bg-white/40"
            />
          </div>
          ...
          <div className="bg-blue-600/10 border border-blue-600/20 rounded-2xl p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-2.5">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-white/10 disabled:text-slate-400 text-white font-bold text-xs rounded-full transition-all shadow-xs cursor-pointer border border-white/20"
              >
            ...
            {aiEstimationResult && (
              <div className="bg-white/50 border border-white/30 backdrop-blur-md rounded-xl p-4.5 text-xs shadow-2xs animate-in slide-in-from-top-1">
                ...
                <p className="text-slate-600 leading-relaxed italic font-semibold">
          ...
          <div>
            ...
            {isEditingDesc ? (
              <textarea
                ...
                className="w-full h-48 px-4 py-3 text-xs glass-input rounded-2xl font-mono focus:bg-white/50 text-slate-800"
                placeholder="Use Markdown formats: # Header, **bold**, - list..."
              />
            ) : (
              <div className="prose prose-sm prose-slate max-w-none bg-white/40 backdrop-blur-md rounded-2xl p-5 border border-white/30 min-h-[120px] text-slate-800 text-xs leading-relaxed space-y-2 whitespace-pre-wrap font-sans shadow-2xs font-medium">
                {description || (
          ...
          <div>
            <div className="flex items-center justify-between mb-2.5">
              ...
              {issue.subtasks.length > 0 && (
                <span className="text-xs font-bold text-slate-700 bg-white/40 border border-white/30 px-2 py-0.5 rounded-full text-[11px] shadow-2xs">
            ...
            <div className="space-y-1.5 mb-3">
              {issue.subtasks.map((st) => (
                <div key={st.id} className="flex items-center justify-between p-2.5 bg-white/30 hover:bg-white/50 border border-white/20 rounded-xl group text-xs transition-colors">
                  <div className="flex items-center gap-2.5 min-w-0 flex-1">
                    <input
                      type="checkbox"
                      checked={st.completed}
                      onChange={(e) => handleToggleSubtask(st.id, e.target.checked)}
                      className="w-4 h-4 text-blue-600 rounded border-white/40 focus:ring-blue-500/20 focus:ring-2 cursor-pointer"
                    />
                    <span className={`text-slate-800 font-semibold truncate ${st.completed ? "line-through text-slate-400" : ""}`}>
                      {st.title}
                    </span>
                  </div>
                  <button
          ...
          <div className="border-t border-white/20 pt-6">
            ...
            <form onSubmit={handleAddCommentSubmit} className="flex items-start gap-3 mb-6">
              ...
              <div className="flex-1 space-y-2">
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-xs glass-input rounded-xl placeholder:text-slate-400 shadow-2xs text-slate-800"
                />
                <button
                  type="submit"
            ...
            <div className="space-y-4">
              {issue.comments.map((comment) => (
                <div key={comment.id} className="flex items-start gap-3 text-xs p-4 bg-white/30 rounded-xl border border-white/20 hover:border-blue-600/30 transition-all group">
                  <img
                    src={comment.avatar}
                    alt={comment.author}
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-slate-600 font-semibold leading-relaxed font-sans">{comment.text}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-4 bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl p-5 space-y-5 h-fit">
          ...
          <div className="border-t border-white/20 pt-4">
            ...
            {!isLoggingWork ? (
              ...
              <form onSubmit={handleLogWorkSubmit} className="space-y-2.5 p-3.5 bg-white/30 border border-white/20 rounded-2xl shadow-2xs">
                <div>
                  <input
                    type="number"
                    step="0.5"
              ...
              <div className="mt-3.5 max-h-[140px] overflow-y-auto space-y-2 divide-y divide-white/20 scrollbar-thin">
                {issue.workLogs.map((wl) => (
                  <div key={wl.id} className="text-[10px] text-slate-500 pt-2.5 first:pt-0">
                    ...
                    <p className="italic text-slate-600 font-semibold truncate mt-0.5">"{wl.description}"</p>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Time Dates stamp */}
          <div className="border-t border-white/20 pt-4 text-[9px] text-slate-500 font-bold space-y-1">
